...
def make_data_loader(cfg, is_train=True, is_distributed=False, start_iter=0, is_for_period=False):
    ...
    for dataset in datasets:
        ...
        # BBoxAugCollator：从数据集的样本列表中，返回图像和目标。图片应该在im_detect_bbox_aug中转换成成批的图片。
        # _C.DATALOADER.SIZE_DIVISIBILITY = 0
        # BatchCollator：从数据集的样本列表中，返回已批处理的图像和目标
        ...
        data_loader = torch.utils.data.DataLoader(
            ...
            batch_sampler=batch_sampler, # 批量采样，每次返回的是一批数据的索引
            collate_fn=collator, # 将一小段数据合并成数据列表
        )
